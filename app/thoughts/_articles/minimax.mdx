export const metadata = {
  title: 'DRAFT: Minimax for Tic Tac Toe',
  date: '2025.12.27',
...
After that, the implementation is just two symmetric loops. 
We iterate over every legal move, simulate it, recurse to get a score for the resulting position, then undo the move. 
The only difference is whether we keep the maximum score (X’s turn) or the minimum score (O’s turn):
...
Minimax returns a score, not the move itself. 
So to pick a move, we try every legal move for X, run Minimax on it, and keep the one with the highest score:
