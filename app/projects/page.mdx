export const metadata = {
  title: 'Projects',
  description:
    'Selected work across backend systems, developer tools, and applied AI.',
}

# Projects

I build product-focused software across backend systems, developer tools, and applied AI. I like projects that ship: real users, real constraints, and clean interfaces.

import { Plaintext } from '@/components/project-logos'
import { SkillChips } from '@/components/skill-chips'

---

## [<Plaintext title="Canvas MCP" />](https://github.com/ynbh/canvasmcp) · Canvas LMS MCP Server

MCP server and CLI that exposes Canvas coursework data and workflows to AI clients.

![Canvas MCP Architecture](diagrams/canvas-mcp.svg)

- Exposes assignments, discussions, files, grades, modules, and calendar data to LLM clients
- Implements **25+ tools**, course-context snapshots, and URL resolution for assignment-aware workflows
- Supports browser-cookie, session-cookie, and API-token auth across MCP and CLI flows

<SkillChips skills={["Python", "FastMCP", "Canvas API", "Typer"]} />

---

## [<Plaintext title="Codexport" />](https://github.com/ynbh/codexport) · Codex Session Export Tool

Rust CLI and local web app for browsing Codex chat history and exporting sessions to clean Markdown.

- Parses local Codex data from SQLite and renders transcripts for review, reuse, and documentation
- Ships a local Axum interface for browsing sessions without digging through raw app storage
- Supports clean Markdown export and print-friendly transcript workflows

<SkillChips skills={["Rust", "Axum", "Rusqlite", "Comrak", "Tokio"]} />

---

## [<Plaintext title="lc" />](https://github.com/ynbh/lc) · LeetCode Workflow CLI

Rust CLI for LeetCode practice loops, from problem discovery to local editing, example runs, guarded checks, and real submissions.

![lc Workflow](diagrams/lc.svg)

- Uses public LeetCode metadata for daily challenges, problem lookup, lists, and profile views
- Probes browser cookies for authenticated judge runs and submissions without a manual token-copying workflow
- Creates starter solution files with metadata headers so `test`, `check`, `run`, and `submit` can infer the target problem

<SkillChips skills={["Rust", "Clap", "Reqwest", "Tokio", "LeetCode API"]} />

---

## [<Plaintext title="Piazza CLI" />](https://github.com/ynbh/piazza-cli) · Terminal Piazza Reader

Read-only Rust CLI for browsing Piazza classes, feeds, unread posts, and recent-thread search from a terminal.

![Piazza CLI Flow](diagrams/piazza-cli.svg)

- Copies Piazza browser cookies into a local `session.json` with `chrome`, `edge`, `firefox`, and `safari` support
- Lists classes, reads feeds, fetches posts from class/post IDs or URLs, and extracts tags/unread threads
- Supports `pretty`, `json`, `jsonl`, and `ids` output for human reading or shell pipelines

<SkillChips skills={["Rust", "Clap", "Reqwest", "cookie-scoop", "Piazza API"]} />

---

## [<Plaintext title="Testudot" />](https://github.com/ynbh/testudot) · Course Change Monitor

Backend service monitoring UMD course availability with automated alerts.

![Testudot Monitoring Flow](diagrams/testudot.svg)

- Dockerized FastAPI service that sends HTML alerts when course schedule state changes
- Detects new sections, seat-availability changes, and removals through state diffing
- Uses local JSON persistence in development and Redis-backed storage in production

<SkillChips skills={["Python", "FastAPI", "Typer", "Redis", "Docker"]} />

---

## [<Plaintext title="Know" />](https://github.com/ynbh/know) · Local Semantic Search CLI

Hybrid search CLI with persistent vector storage for local document search.

![Know RAG Pipeline](diagrams/know.svg)

- Watches folders, chunks **20+ file types**, and stores a persistent Chroma index for local retrieval
- BM25 + RRF fusion for higher-quality hybrid search results
- Reduced re-indexing time by **67%** through incremental indexing and chunk deduplication

<SkillChips skills={["Python", "LlamaIndex", "Chroma", "BM25"]} />

---

## [<Plaintext title="Walker" />](https://github.com/ynbh/walker) · Concurrent Link Validator

High-performance Rust CLI that validates URLs in parallel.

![Walker Flow](diagrams/walker.svg)

- Validates **500+ links in &lt;10 seconds** using Tokio async runtime
- Connection pooling with configurable thread count
- Published to **[crates.io](https://crates.io/crates/link-walker)**

<SkillChips skills={["Rust", "Tokio", "reqwest", "CLI"]} />

---

## [<Plaintext title="tsr" />](https://github.com/ynbh/tsr) · Terminal Transcription CLI

Local audio/video transcription via whisper.cpp with auto model management.

![tsr Pipeline](diagrams/tsr.svg)

- Auto-downloads and manages Whisper models (`tiny` -> `large`)
- Supports **12+ audio/video formats** and any URL yt-dlp supports
- Outputs SRT subtitles, JSON, or plaintext

<SkillChips skills={["Python", "whisper.cpp", "FFmpeg", "CLI"]} />

---

## [<Plaintext title="PlanetTerp Client" />](https://github.com/ynbh/planet-terp-client) · API Client Library

Typed Python client for PlanetTerp API. Published to **[PyPI](https://pypi.org/project/planet-terp-client/)**.

- Clean API: `course()`, `professor()`, `search()`, `grades()`
- Type-safe dataclass models
- Includes OpenAPI spec and demo scripts

<SkillChips skills={["Python", "PyPI", "REST API"]} />

---

### Other Projects

- **[Watch](https://github.com/ynbh/watch)**: Terminal-first show search + playback. A Textual TUI that searches TMDB, browses results, and launches ad-less playback.
- **[Rename](https://github.com/ynbh/rename)**: Batch renamer that walks a directory and asks a model for better file names.
- **[Snippet-Verse](https://github.com/ynbh/snippet-verse)**: Stateless code sharing via URL encoding. Deployed on Vercel.
